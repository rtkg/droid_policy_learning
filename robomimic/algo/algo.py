...
class PolicyAlgo(Algo):
    ...
    def compute_batch_visualize(self, batch, num_samples, savedir=None):
        ...
        varied_cam_1_images = batch["obs"]["camera/image/varied_camera_1_left_image"][
            :num_samples
        ][:, 0, :, :, :]
        varied_cam_2_images = batch["obs"]["camera/image/hand_camera_left_image"][
            :num_samples
        ][:, 0, :, :, :]
        images = {
            "varied_camera_1_image": varied_cam_1_images,
            "hand_camera_left_image": varied_cam_2_images,
        }
    ...
    def compute_mse_visualize(self, trainset, validset, num_samples, savedir=None):
        ...
        if len(eval_data) < int(num_samples / 2):
            ...
            training_sampled_data = random.sample(
                eval_data, int(num_samples / 2)
            ) + random.sample(broad_data, int(num_samples / 2))
...
class RolloutPolicy(object):
    ...
    def __call__(self, ob, goal=None):
        ...
        ac = self.policy.get_action(
            obs_dict=ob, goal_mode=self.goal_mode, eval_mode=self.eval_mode
        )
